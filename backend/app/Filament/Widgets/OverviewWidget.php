...
final class OverviewWidget extends Widget
{
    ...
    protected function getViewData(): array
    {
        ...
        return [
            'stats' => [
                $this->stat('Open jobs', number_format(Job::query()->where('status', 'open')->count()), null,
                    'Awaiting an offer or quote', 'up', $this->spark($this->dailySeries(Job::query(), 'created_at'))),
                $this->stat('Active engagements', number_format(Engagement::query()->whereNull('completed_at')->count()), null,
                    'In flight, not yet completed', 'up', $this->spark($this->dailySeries(Engagement::query(), 'accepted_at'))),
                $this->stat('Escrow held', $this->money($ledger->totalByKindMinor(AccountKind::EscrowLiability)), 'FCFA',
                    'Owed to providers on completion', 'flat', null, 'info'),
                $this->stat('GMV · 30 days', $this->money((int) Engagement::query()->where('accepted_at', '>=', now()->subDays(30))->sum('agreed_amount_minor')), 'FCFA',
                    'Agreed value of new engagements', 'up', null),
                $this->stat('Platform revenue', $this->money($ledger->totalByKindMinor(AccountKind::PlatformRevenue)), 'FCFA',
                    'Commission earned to date', 'up', null),
                $this->stat('Reconciliation exceptions', number_format($openExceptions), null,
                    $openExceptions === 0 ? 'Ledger matches — all clear' : 'Open · needs a human',
