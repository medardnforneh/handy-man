 ...
 * so a person's hundredth call of the day costs one cache read, not an upsert.
 *
 * Best effort, and that has to be enforced here, not assumed: an exception thrown from a
 * terminable middleware is rendered by the framework AFTER the response body has been written, so
 * it arrives on the wire appended to the JSON the client is parsing. A missing table turned every
 * API response into `{...}{"message": "SQLSTATE..."}` — found because the force-update screen could
 * not read its own 426. Nothing thrown in `terminate` may escape; it is reported and dropped.
 *
