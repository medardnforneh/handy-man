...
- [x] Workspace state converges after a hard socket kill mid-session — verified on the real failing cycle (BUILD_STATE, P4-07 entry): thread loaded → Reverb killed → message posted over the API → Reverb restarted → the missed message appears without a reload, the channel reports `subscribed: true` again, a message posted after the reconnect arrives live, no duplicates. Reconciliation hangs off channel re-subscription and always tears down and rejoins the channel (a restart leaves pusher-js's channel silently dead otherwise). Manual evidence, not an automated test — the three failure modes it took to get there are written up in that entry
...
- [x] `pro` app performs a check-in with zero connectivity and syncs exactly once on reconnect — the real dev build driven in headless Chrome (2026-09-12): work detail loaded online, network cut with `Network.emulateNetworkConditions`, Check in tapped → "You're offline · 1 waiting to send" and the optimistic checked-in state; network restored → **one** POST reaches the API under the **same** Idempotency-Key the offline attempt carried, and `work_sessions` holds exactly one row with the fix taken at the moment of the tap. The queue's semantics are `write-queue.service.spec.ts` (replay exactly once, same key; a lost response after the server accepted; order; refusal; logout), which now runs in CI (`npm run test:ci`) — it had passed locally for weeks while nothing ran it. Still owed on a physical Android: the same run through the packaged APK
...
- [x] **A customer can find a provider and request a quote without loading the app bundle** (Blade) — `/services/{trade}/request`: form → one-time code → posted, server-rendered, no JavaScript, on the same actions the app calls (`RequestOtp`, `VerifyOtp`, `RecordConsent`, `CreateAddress`, `CreateJob`, `PublishJob`); on-site requests are placed at the chosen city's centroid (`config/cities.php`). `PublicQuoteRequestTest` (8 scenarios: remote and on-site end to end, validation, wrong code, rate limit, cross-trade draft) and a headless-Chrome run of the real pages (2026-09-12)
