...
- **WhatsApp and SMS adapters** do not exist: `notifications.php` offers `fake` and `log` only.
  The interfaces (`WhatsAppSender`, `SmsSender`) and the channel ladder are in place; the Meta
  Cloud API and an SMS aggregator adapter are the two pieces of code the "templates approved" items
  are waiting on as much as on Meta.
